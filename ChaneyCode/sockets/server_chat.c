...
main(
  ...
  for ( ; ; ) {
    ...
    if ((n = read(sockfd, buf, sizeof(buf))) == 0) {
      ...
      // Instead of simply echoing back to the client the text that was sent,
      // read from new text from stdin (using fgets()) and send that back to
      // the client. Setup a descent prompt for the server side.
